...
async function seedUsers() {
    ...
    for (let i = 0; i < 50; i++) {
        const user = {
            username: faker.internet.userName().replace(/[^a-zA-Z0-9]/g, '') + faker.number.int(1,999),            
        };
        users.push(user);
    }
    ...
    console.log(`${users.length} users and ${messages.length} messages have been seeded.'`);
}
